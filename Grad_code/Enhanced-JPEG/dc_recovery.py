...
def read_image(image_path):
    ...
    cv2.imwrite('images/input.png', image)
    return image
...
def dc_recovery(image_path):
    ...
    for i in range(img.shape[2]):
        ...
        dc_preset = [dct_coefs_corner[0, 0, 0, 0], dct_coefs_corner[0, h_n - 1, 0, 0],
                     dct_coefs_corner[w_n - 1, 0, 0, 0],
                     dct_coefs_corner[w_n - 1, h_n - 1, 0, 0]]  # ,dct_coefs_corner[0, h_n + 1, 0, 0]]
        dct_preds = dc_prediction_1(dct_coefs, dc_preset[0])
        dct_preds += dc_prediction_2(dct_coefs, dc_preset[1])*2
        dct_preds += dc_prediction_3(dct_coefs, dc_preset[2])*2
        dct_preds += dc_prediction_4(dct_coefs, dc_preset[3])
        # dct_preds += dc_prediction_5(dct_coefs, dc_preset[4])
        dct_preds /= 6.
        image_rec.append(recover_from_dct(dct_preds, mode='compress'))
    # img_recover
    # for i in range(image_rec):
    img_recover = np.stack((image_rec[0], image_rec[1], image_rec[2]), axis=2)
    # print(image_rec.shape)
    # print(dct_preds.shape)
    # input()
    return dct_preds, img_recover


if __name__ == '__main__':
    argparser = argparse.ArgumentParser("DC recovery")
    argparser.add_argument('--in-path', type=str, default='./images/lenna_HR.png', help="Input image path.")
    argparser.add_argument('--out-path', type=str, default='./images/lenna_mine.png', help="Output image path.")
    ...
    img_shape = cv2.imread(args.in_path)
    image_rec = cv2.resize(image_rec, (img_shape.shape[0], img_shape.shape[1]))
    image_rec = cv2.resize(image_rec, (512, 512))
