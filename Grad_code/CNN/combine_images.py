...
# parser.add_argument('writeable'    , type=bool , default=False)
...
def make_quality_compression(original,sal):
    ...
    uncompressed_size = os.path.getsize(r'E:\pythonProjects\CNN_img_compression\data\images\0.png')
    # os.remove(args.output_directory + '/temp.png')

    out_img = array2PIL(k)

    if args.find_best:
        out_name = args.output_directory + '/' + '_compressed_' + args.image.split('/')[-1] + '_' + '.png'
...
#         args.map   = map_file
original = Image.open(r'E:\pythonProjects\CNN_img_compression\data\images\0.png')
# print(np.array(original).shape)
sal = Image.open('output/msroi_map.jpg')
a,b = make_quality_compression(original,sal)
# if args.print_metrics:
#     get_metrics(args.image,a,b, original.size)
